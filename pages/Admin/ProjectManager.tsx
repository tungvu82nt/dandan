            ...
            <tbody className="text-sm text-gray-700">
                {filtered.map(p => (
                    <tr key={p.id} className="hover:bg-gray-50 border-b last:border-0">
                        ...
                        <td className="p-4">
                            <span className={`px-2 py-1 rounded text-xs ${p.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                                {p.status === 'active' ? '募捐中' : '已结束'}
                            </span>
                        </td>
                        <td className="p-4">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-primary"
                >
                  <option value="active">募捐中 (Active)</option>
